...
async function handleRegister(req, res) {
  ...
  try {
    ...
    const created = await prisma.$transaction(async (tx) => {
      const partner = await tx.partner.create({ data: { name, type, city } });
    ...
    setAuthCookie(res, token);
    res.status(200).json({ success: true, partner: { id: created.id, name: created.name, type: created.type, city: created.city } });
...
export default async function handler(req, res) {
  ...
  // URL: /api/auth/login, /api/auth/register, /api/auth/me, /api/auth/logout
