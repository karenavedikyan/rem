...
const normalizeItemKind = (value) => {
  const v = asString(value).toUpperCase();
  return ITEM_KINDS.has(v) ? v : null;
};
...
const mapService = (service) => ({
  ...
  itemKind: (service.itemKind || "SERVICE"),
  stage: service.stage || null,
  taskType: service.taskType || null,
...
export async function updatePartnerById(partnerId, payload) {
  ...
  const REMOD_FIELDS = ["name", "type", "description", "city", "specializations", "areas"];

  const fromPrisma = await runWithPrisma(async (prisma) => {
    const partner = await ensureDemoPartner(prisma, partnerId);
    if (!partner) return null;

    const current = await prisma.partner.findUnique({ where: { id: partnerId }, select: { isApproved: true } });
    if (current?.isApproved) {
      const changedRemod = REMOD_FIELDS.some((f) => data[f] !== undefined);
      if (changedRemod) {
...
export function validateNewServicePayload(payload) {
  const raw = isObject(payload) ? payload : {};
  const itemKind = normalizeItemKind(raw.itemKind) || "SERVICE";
  const title = asString(raw.title);
  if (!title) throw new Error("INVALID_TITLE");

  let stage = normalizeStage(raw.stage);
  let taskType = normalizeTaskType(raw.taskType);
...
export function validateServicePatchPayload(payload) {
  ...
  if (raw.itemKind != null) {
    const itemKind = normalizeItemKind(raw.itemKind);
    if (!itemKind) throw new Error("INVALID_ITEM_KIND");
    data.itemKind = itemKind;
  }
  if (raw.stage !== undefined) {
    const str = asString(raw.stage);
    if (!str) {
      data.stage = null;
    } else {
  ...
  if (raw.taskType !== undefined) {
    const str = asString(raw.taskType);
    if (!str) {
      data.taskType = null;
    } else {
  ...
  const fromPrisma = await runWithPrisma(async (prisma) => {
    ...
    const created = await prisma.service.create({
      data: {
        ...
        itemKind: data.itemKind || "SERVICE",
        stage: data.stage,
