...
const mapService = (item) => ({
  ...
  partner: {
    ...
    description: item.partner.description ?? null,
    phone: item.partner.phone ?? null,
    promotionBannerUrl: item.partner.promotionBannerUrl ?? null,
    specializations: Array.isArray(item.partner.specializations) ? item.partner.specializations : [],
    areas: Array.isArray(item.partner.areas) ? item.partner.areas : []
  }
});
...
export function normalizeCatalogFilter(query = {}) {
  ...
  return {
    ...
    partnerId: asString(query.partnerId) || undefined
  };
}
...
export async function listCatalogServices(filter, pagination) {
        ...
        include: {
          partner: {
            select: {
              ...
              promotionBannerUrl: true, description: true, phone: true,
              specializations: true, areas: true
            }
          }
        },
  ...
  const filtered = state.services.filter((item) => {
    ...
    if (filter.partnerId && (item.partnerId !== filter.partnerId && item.partner?.id !== filter.partnerId)) return false;
