...
## Архитектура публичной навигации (этап 1)

Primary-меню строится вокруг клиентского сценария:
...
Secondary-разделы вынесены в footer:
...
Подробная карта маршрутов: `docs/route-map.md`.
...
- Кнопка «Оставить заявку на эту услугу» ведёт на `/request/` и передаёт `serviceId/serviceTitle` через query.
